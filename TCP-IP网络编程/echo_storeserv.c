...
int main(int argc, char *argv[])
{
    ...
    if (pid == 0)
    {
        FILE *fp = fopen("echomsg.txt", "w"); // wt为二进制
        char msgbuf[BUF_SIZE * 2];
        int len;
        ...
        // for(int i=0; i<10; i++)
        // {
        //     read(fds[0], msgbuf, 2*BUF_SIZE);
        //     printf("Received message from client(w): %s\n", msgbuf);
        ...
        for (int i = 0; i < 10; i++) {
            ssize_t bytesRead = read(fds[0], msgbuf, 2 * BUF_SIZE);
            
            if (bytesRead > 0) {
                msgbuf[bytesRead] = '\0';  // Null-terminate the received data
                printf("Received message from client(w): %s\n", msgbuf);
                fputs(msgbuf, fp);
                fputc('\n', fp);
            } else if (bytesRead == 0) {
